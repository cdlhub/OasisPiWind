node {
    ...
    try {
        ...
        stage('Test Keyserver: ' + model_func) {
            dir(build_workspace) {
                sh PIPELINE + " run_keyserver ${env.MODEL_VARIENT} ${env.VERS_KEYS_DATA}"
            }
        }
        if(params.RUN_ANALYSIS){
            stage('Test analysis: ' + model_func) {
                dir(build_workspace) {
                    sh PIPELINE + " run_analysis ${env.MODEL_VARIENT} ${env.MODEL_SUPPLIER}"
                }
            }
        }
        if(params.CHECK_ANALYSIS){
            stage('Check analysis output: ' + model_func) {
                dir(build_workspace) {
                    sh PIPELINE + " run_output_comparison ${env.MODEL_VARIENT} ${model_test_case}"
                }
            }
        }
        if (params.PUBLISH){
